...
describe("GameRenderLayers", () => {
  it("keeps HUD in front of game objects and result UI at the very front", () => {
    const root = new createjs.Container();
    const layers = new GameRenderLayers();

    layers.attachTo(root);

    expect(root.children).toEqual([layers.world, layers.combatFeedback, layers.hud, layers.result]);
  });
  ...
  it("keeps newly spawned game objects behind HUD and result layers", () => {
    ...
    expect(root.children.at(-2)).toBe(layers.hud);
    expect(root.children.at(-1)).toBe(layers.result);
